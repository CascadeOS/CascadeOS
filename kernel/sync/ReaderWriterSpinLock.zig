...
pub fn writeLock(self: *ReaderWriterSpinLock) kernel.sync.SpinLock.Held {
    const held = self.lock.lock();
...
pub fn writeUnlock(self: *ReaderWriterSpinLock, held: kernel.sync.SpinLock.Held) void {
    _ = self;
    held.unlock();
}

pub fn upgradeReadToWriteLock(self: *ReaderWriterSpinLock) kernel.sync.SpinLock.Held {
    self.readUnlock();
