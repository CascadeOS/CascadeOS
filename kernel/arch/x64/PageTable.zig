...
pub const PageTable = extern struct {
    ...
    inline fn entries(page_table: *PageTable) []volatile Entry.Raw {
        return &page_table._entries;
    }

    inline fn entriesConst(page_table: *const PageTable) []const volatile Entry.Raw {
        return &page_table._entries;
    }
    ...
    pub fn load(physical_page: cascade.mem.PhysicalPage.Index) void {
    ...
    pub fn copyTopLevel(source: *PageTable, destination: *PageTable) void {
        if (core.is_debug) std.debug.assert(source != destination);
        @memcpy(destination.entries(), source.entriesConst());
    }
    ...
    pub fn mapSinglePage(
        ...
        const level3_table, const created_level3_table = try level4_entries[level4_index].ensureNextTable(
        ...
        const level2_table, const created_level2_table = try level3_entries[level3_index].ensureNextTable(
            physical_page_allocator,
        );
        errdefer {
        ...
        const level1_table, const created_level1_table =
            try level2_entries[level2_index].ensureNextTable(physical_page_allocator);
        errdefer {
        ...
        try level1_entries[p1Index(virtual_address)].setEntry(
            physical_page.baseAddress(),
            map_type,
            .small,
        );
    }
    ...
    pub fn print(
        ...
        for (page_table.entriesConst(), 0..) |raw_level4_entry, level4_index| {
            ...
            for (level3_table.entriesConst(), 0..) |raw_level3_entry, level3_index| {
                ...
                for (level2_table.entriesConst(), 0..) |raw_level2_entry, level2_index| {
                    ...
                    for (level1_table.entriesConst(), 0..) |raw_level1_entry, level1_index| {
    ...
    pub const init = struct {
        ...
        pub fn sizeOfTopLevelEntry() core.Size {
            return level_4_address_space_size; // TODO: Only correct for 4 level paging
        }
        ...
        pub fn fillTopLevel(
            ...
            _ = try raw_entry.ensureNextTable(physical_page_allocator);
        }
        ...
        pub fn mapToPhysicalRangeAllPageSizes(
            ...
            while (level4_index <= last_virtual_address_p4_index) : (level4_index += 1) {
                const level3_table, _ = try level4_entries[level4_index].ensureNextTable(
                    physical_page_allocator,
                );
                ...
                while (level3_index <= last_level3_index) : (level3_index += 1) {
                        ...
                        current_physical_address.aligned(large_page_size_alignment))
                    {
                        ...
                        // large 1 GiB page
                        try level3_entries[level3_index].setEntry(
                    ...
                    const level2_table, _ = try level3_entries[level3_index].ensureNextTable(
                        physical_page_allocator,
                    );
                    ...
                    while (level2_index <= last_level2_index) : (level2_index += 1) {
                        ...
                        {
                            // large 2 MiB page
                            try level2_entries[level2_index].setEntry(
                                current_physical_address,
                                map_type,
                                .medium,
                            );
                        ...
                        const level1_table, _ = try level2_entries[level2_index].ensureNextTable(
                            physical_page_allocator,
                        );
                        ...
                        while (level1_index <= last_level1_index) : (level1_index += 1) {
                            try level1_entries[level1_index].setEntry(
                                current_physical_address,
                                map_type,
                                .small,
                            );
...
const Entry = extern union {
    ...
    const Raw = extern struct {
        ...
        fn ensureNextTable(
            raw_entry: *volatile Raw,
            physical_page_allocator: cascade.mem.PhysicalPage.Allocator,
        ) cascade.mem.MapError!struct { *PageTable, bool } {
...
const level_4_address_space_size = core.Size.from(512, .gib);
const level_1_address_space_size = PageTable.small_page_size;
const level_2_address_space_size = PageTable.medium_page_size;
const level_3_address_space_size = PageTable.large_page_size;

const level_4_address_space_size_alignment = level_4_address_space_size.toAlignment();

const small_page_size_alignment = PageTable.small_page_size.toAlignment();
const medium_page_size_alignment = PageTable.medium_page_size.toAlignment();
const large_page_size_alignment = PageTable.large_page_size.toAlignment();
