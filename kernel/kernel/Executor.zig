...
pub fn renderInterruptSourcePanicMessage(
    ...
    var bw: std.Io.Writer = .fixed(full_buffer[0..kernel.config.executor.interrupt_source_panic_buffer_size.value]);

    bw.print(fmt, args) catch {
        @memcpy(
            full_buffer[kernel.config.executor.interrupt_source_panic_buffer_size.value..],
