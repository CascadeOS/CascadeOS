...
pub fn drop(context: *kernel.Context) noreturn {
    if (core.is_debug) {
    ...
    kernel.scheduler.drop(context, .{
        .action = struct {
            fn action(new_context: *kernel.Context, old_task: *kernel.Task, _: usize) void {
...
inline fn setExecutor(context: *kernel.Context) void {
...
const log = kernel.debug.log.scoped(.context);
