...
range_allocator: kernel.vmm.VirtualRangeAllocator = .{},
...
pub fn init(total_range: core.VirtualRange) MemoryRegionPool.GetError!AddressSpace {
    ...
    return .{
        .range_allocator = try kernel.vmm.VirtualRangeAllocator.init(total_range),
    };
}

pub const AllocateError = error{
    AddressSpaceExhausted,
} || MemoryRegionPool.GetError;
...
pub fn allocate(self: *AddressSpace, size: core.Size, map_type: kernel.vmm.MapType) AllocateError!core.VirtualRange {
    core.assert(size.value != 0);
    core.assert(size.isAligned(kernel.arch.paging.standard_page_size));

    const virtual_range = self.range_allocator.allocateRange(size) catch
        return AllocateError.AddressSpaceExhausted;
...
pub fn deallocate(self: *AddressSpace, range: core.VirtualRange) void {
    core.assert(range.size.value != 0);
...
) MemoryRegionPool.GetError!void {
...
fn deallocateMemoryRegion(self: *AddressSpace, range: core.VirtualRange) MemoryRegionPool.GetError!void {
...
fn memoryRegionContainsRangeCompare(range: core.VirtualRange, other_node: *const RedBlack.Node) core.OrderedComparison {
