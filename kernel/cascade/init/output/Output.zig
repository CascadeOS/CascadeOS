...
fn tryGetSerialOutputFromGenericSources(memory_system_available: bool) ?cascade.init.Output {
    const static = struct {
        var init_output_uart: uart.Uart = undefined;
    };

    blk: {
        if (cascade.acpi.tables.SPCR.init.tryGetSerialOutput(memory_system_available)) |output_uart| {
        ...
        if (cascade.acpi.tables.DBG2.init.tryGetSerialOutput(memory_system_available)) |output_uart| {
