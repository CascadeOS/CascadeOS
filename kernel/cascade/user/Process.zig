...
pub fn createThread(
    ...
    cascade.debug.log.scoped(.user_thread).debug("created thread: {f}", .{thread});
...
const globals = struct {
    ...
    var cache: cascade.mem.cache.Cache(
        ...
        .{
            .constructor = struct {
                fn constructor(process: *Process) cascade.mem.cache.ConstructorError!void {
                    ...
                    const page = cascade.mem.PhysicalPage.allocator.allocate() catch |err| {
                        log.warn("process constructor failed during page allocation: {t}", .{err});
                        return error.ItemConstructionFailed;
                    };
                    errdefer {
                        var page_list: cascade.mem.PhysicalPage.List = .{};
                        page_list.push(page);
