...
pub const Index = enum(u32) {
    ...
    pub inline fn range(index: Index) cascade.PhysicalRange {
        return .from(index.baseAddress(), arch.PageTable.standard_page_size);
    }
};
...
fn allocate() Allocator.AllocateError!Index {
    ...
    if (core.is_debug) {
        const virtual_range: cascade.KernelVirtualRange = .from(
            ...
            arch.PageTable.standard_page_size,
        );
...
pub const init = struct {
    ...
    pub fn initializeBootstrapAllocator() void {
        ...
        while (memory_map.next()) |entry| {
            ...
            init_globals.bootstrap_physical_regions.append(.{
                ...
                .page_count = @intCast(entry.range.size.divide(arch.PageTable.standard_page_size)),
    ...
    pub fn mapPagesArray(
        kernel_page_table: arch.PageTable,
    ...
    pub fn initializePhysicalMemory(pages_range: cascade.KernelVirtualRange) void {
        ...
        for (bootstrap_regions.constSlice()) |bootstrap_region| {
            ...
            free_memory.addInPlace(arch.PageTable.standard_page_size.multiplyScalar(free_pages));
            ...
            if (init_log.levelEnabled(.debug)) {
                if (in_use_pages == 0) {
                    init_log.debug(
                        ...
                        .{
                            free_pages,
                            arch.PageTable.standard_page_size.multiplyScalar(free_pages),
                        },
                    );
                    ...
                    init_log.debug(
                        ...
                        .{
                            in_use_pages,
                            arch.PageTable.standard_page_size.multiplyScalar(in_use_pages),
                        },
                    );
                } else {
                    init_log.debug(
