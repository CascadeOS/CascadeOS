...
pub inline fn kernelPageTable() arch.PageTable {
...
///
/// **REQUIREMENTS**:
/// - `virtual_address` must be aligned to `arch.PageTable.standard_page_size`
...
///
/// **REQUIREMENTS**:
/// - `virtual_range.address` must be aligned to `arch.PageTable.standard_page_size`
/// - `virtual_range.size` must be aligned to `arch.PageTable.standard_page_size`
...
pub fn mapRangeAndBackWithPhysicalPages(
    ...
    // TODO: this can be optimized by implementing `arch.mapRangeAndBackWithPhysicalPages`
...
/// - `virtual_range.address` must be aligned to `arch.PageTable.standard_page_size`
/// - `virtual_range.size` must be aligned to `arch.PageTable.standard_page_size`
/// - `physical_range.address` must be aligned to `arch.PageTable.standard_page_size`
/// - `physical_range.size` must be aligned to `arch.PageTable.standard_page_size`
...
pub fn mapRangeToPhysicalRange(
    ...
    // TODO: this can be optimized by implementing `arch.PageTable.mapRange`
...
pub const safe = struct {
    ...
    pub const Result = struct {
        ...
        pub const Slot = struct {
            ...
            pub fn trySignalFailure(slot: *Slot, interrupt_frame: arch.Interrupt.Frame) bool {
...
pub const VirtualRangeBatch = struct {
    ...
    /// - `range.address` must be aligned to `arch.PageTable.standard_page_size`
    /// - `range.size` must be aligned to `arch.PageTable.standard_page_size`
...
pub const ChangeProtectionBatch = struct {
    ...
    /// - `range.virtual_range.address` must be aligned to `arch.PageTable.standard_page_size`
    /// - `range.virtual_range.size` must be aligned to `arch.PageTable.standard_page_size`
...
pub const globals = struct {
    ...
    var kernel_page_table: arch.PageTable = undefined;
...
pub const init = struct {
    ...
    fn registerHeaps(kernel_regions: *KernelMemoryRegion.List) void {
        const size_of_top_level = arch.PageTable.init.sizeOfTopLevelEntry();
    ...
    fn registerPages(kernel_regions: *KernelMemoryRegion.List) void {
        ...
        const pages_range = kernel_regions.findFreeRange(
            core.Size.of(PhysicalPage)
                ...
                .alignForward(arch.PageTable.standard_page_size_alignment),
            arch.PageTable.standard_page_size_alignment,
    ...
    fn buildAndLoadKernelPageTable() arch.PageTable {
        const kernel_page_table: arch.PageTable = .create(
            ...
            switch (region.type) {
                .direct_map => {
                    ...
                    while (iter.next()) |entry| {
                        ...
                        arch.PageTable.init.mapToPhysicalRangeAllPageSizes(
                            kernel_page_table,
                ...
                .special_heap,
                .kernel_address_space,
                => arch.PageTable.init.fillTopLevel(
                    kernel_page_table,
                    region.range.toVirtualRange(),
                    PhysicalPage.init.bootstrap_allocator,
                ) catch |err| std.debug.panic("failed to map {f}: {t}", .{ region, err }),
