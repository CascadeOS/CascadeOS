...
pub fn create(size: core.Size) error{OutOfMemory}!*AnonMap {
    if (core.is_debug) std.debug.assert(size.aligned(arch.PageTable.standard_page_size_alignment));
...
pub const Reference = struct {
    ...
    pub fn lookup(reference: Reference, entry: *const Entry, faulting_address: cascade.VirtualAddress) ?*AnonPage {
        if (core.is_debug) {
            ...
            std.debug.assert(reference.start_offset.aligned(arch.PageTable.standard_page_size_alignment));
    ...
    fn targetIndex(entry: *const Entry, reference: Reference, faulting_address: cascade.VirtualAddress) u32 {
        ...
        return @intCast(
            entry.range.address.difference(faulting_address).divide(arch.PageTable.standard_page_size) +
                reference.start_offset.divide(arch.PageTable.standard_page_size),
        );
    }
