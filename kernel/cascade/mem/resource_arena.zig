    ...
    return struct {
            ...
            switch (quantum_caching) {
                ...
                .normal => |count| {
                    ...
                    for (quantum_caches, 0..) |*quantum_cache, i| {
                        var cache_name: cascade.mem.cache.Name = .{};
                ...
                .heap => |count| {
                    ...
                    for (0..frames_to_allocate) |_| {
                        ...
                        const frame_caches = cascade.mem.directMapFromPhysical(frame.baseAddress())
                        ...
                        for (frame_caches) |*cache| {
                            ...
                            cache.init(context, .{
                                .name = cascade.mem.cache.Name.initPrint(
                                    "heap qcache {}",
                                    .{caches_created},
                                ) catch unreachable,
        ...
        pub fn deinit(arena: *@This(), context: *cascade.Context) void {
            ...
            if (quantum_caching.haveQuantumCache()) {
                ...
                switch (quantum_caching) {
                    .no => {},
                    .yes => cascade.mem.heap.allocator.free(arena.quantum_caches.allocation),
                    .heap => cascade.mem.phys.allocator.deallocate(arena.quantum_caches.allocation),
                }
            }
        ...
        const QuantumCaches = struct {
            ...
            const Allocation = switch (quantum_caching) {
                .none => void,
                .normal => []RawCache,
                .heap => cascade.mem.phys.FrameList,
            };
        };
    };
}
